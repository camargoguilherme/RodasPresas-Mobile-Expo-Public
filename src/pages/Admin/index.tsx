...
export default function Admin() {
  ...
  function renderItemAdmin({_id, name, email, blocked, ...rest}: IUser) {
    return (
      <ViewUserInfo>
        <ImageProfile source={sourceProfle(rest)} />
        <UserInfo>
  ...
  function renderItem({_id, name, email, blocked, ...rest}: IUser) {
    return (
      <ButtonUserInfo
        onLongPress={() =>
          user._id !== _id &&
          setSetMakeAdmin({_id, name, email, blocked, ...rest})
        }>
        <ViewUserInfo>
          <ImageProfile source={sourceProfle(rest)}/>
          <UserInfo>
    ...
    return loadingStorage ? (
     ...
     (
      <FlatList
        ...
        renderItem={({item}) => renderItemAdmin(item)}
        keyExtractor={(item) => item._id}
        contentContainerStyle={{ flexGrow: 1 }}
        ItemSeparatorComponent={renderSeparator}
        onRefresh={refreshUser}
        ...
        <Modal>
          ...
          <ViewButtonModal>
            ...
            <Button
              style={{width: '45%', marginTop: 0}}
              onPress={() => {
                makeUserAdmin(makeAdmin._id, !makeAdmin.admin);
                setSetMakeAdmin();
              }}>
              {loading ? (
                <ActivityIndicator size={20} color="#fff" />
